...
const INITIAL_STATE: State = {
  ...
  metrics: { loss: 0, meanLoss: 0, activeCells: 0, residentCells: 0, activeOrganisms: 0, residentOrganisms: 0, dormantOrganisms: 0, activeColonies: 0, activeSynapsesProxy: 0, memoryBytesProxy: 0, resourceScore: 0, events: 0 },
...
export default function App() {
  ...
  return <main>
    ...
    <section className="metrics"><Metric label="Unsupervised loss" value={state.metrics.loss.toFixed(4)} hint="lower is better"/><Metric label="Processing cells" value={`${state.metrics.activeCells}/${state.metrics.residentCells}`} hint="active / resident"/><Metric label="Organism memory" value={`${state.metrics.activeOrganisms}/${state.metrics.residentOrganisms}`} hint="processing / resident"/><Metric label="Dormant memory" value={state.metrics.dormantOrganisms} hint="retained at low compute"/><Metric label="Resource score" value={state.metrics.resourceScore.toFixed(3)} hint="benefit − proxy cost"/><Metric label="State hash" value={state.stateHash}/></section>
    <section className="workspace">
      ...
      <section className="panel ecosystem"><div className="panel-header"><div><p className="panel-title">Living architecture</p><span>Young organisms learn fast; mature experts stabilize; dormant memories wait for familiar information.</span></div><span className="seed">seed {state.seed}</span></div><LivingArchitecture state={state} selectedId={selected?.id ?? null} onSelect={setSelectedId}/><div className="legend"><span><i className="food-dot"/>information food</span><span><i className="dot cyan"/>cell</span><span><i className="dot violet"/>processing organism</span><span><i className="dormant-dot"/>dormant memory</span><span><i className="ring"/>persistent colony</span></div></section>
