...
export type State = {
  ...
  metrics: { loss: number; meanLoss: number; activeCells: number; residentCells: number; activeOrganisms: number; residentOrganisms: number; dormantOrganisms: number; activeColonies: number; activeSynapsesProxy: number; memoryBytesProxy: number; resourceScore: number; events: number };
  cells: Cell[];
  organisms: Organism[];
  colonies: Colony[];
